...
function LangDropdown({ compact }: LangDropdownProps) {
  ...
  return (
    <div style={{ position:"relative", zIndex:9999 }} onMouseEnter={enter} onMouseLeave={leave}>
      <button style={btnStyle}>
        {compact ? <>{cur?.flag} ▾</> : <>{cur?.flag} {cur?.name} ▾</>}
      </button>
      {open && (
        <div style={{ position:"absolute", right:isRTL?"auto":0, left:isRTL?0:"auto", top:32, background:"rgba(12,20,69,.95)", border:"1px solid rgba(147,197,253,.2)", borderRadius:12, padding:8, zIndex:99999, minWidth:200, boxShadow:"0 8px 32px rgba(0,0,0,.4)" }}>
          {LANGS.map(x => (
